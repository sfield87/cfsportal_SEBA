      ...
      <div class="container-fluid">
        ...
        <div class="row">
          <form id="myform" action="" method="post" class="formulario">
            ...
            <input type="submit" name="RequestInvoice" value="Request Invoices" class="btn btn-go" onclick="this.form.action='BCCForm.jsp?accion=consulta'" style="border: 2px solid #48D597">
            <input id="hist" name="histo" type="checkbox" vaue="Historic Credit">  <label class="form-check-label" for="exampleCheck1">Historic Invoice</label>
             &emsp; Credit ID:
            <input id="CredIn" type="text" name="txtCredit" size="10" placeholder="Credit ID" autofocus />
            <input onclick='javascript:printChecked();return false;' type="submit" name="FindSelected" value="Find Selected" class="btn btn-go" style="border: 2px solid #48D597">
          </form>
        </div>
        <!---<table id="mytable">-->
          ...
          // sta = cnx.createStatement();  //inicializo conex
           ...
           while(rs.next()){
              ...
              <tr>
                ...
                <td class="link" name="BRN" id="link"><a href="BCC_Result.jsp?txtBRN=<%=rs.getString(7)%>"target="_blank"><%=rs.getString(7)%></a></td>
		...
		for(var i=0; i<items.length; i++){
			if(items[i].type=='checkbox' && items[i].checked==true){
				selectedItems+=items[i].value+"\n";
                                if(selectedDocuments==""){selectedDocuments+=documentos[i].innerText;}else{selectedDocuments+="," + documentos[i].innerText;}
                            }
		}
